...
<nobr>Workers & Pages</nobr> ⇢ Create an application ⇢ [Clone a repository](https://deploy.workers.cloudflare.com/?url=https://github.com/andesco/add-magnet-link): \
   `https://github.com/andesco/add-magnet-link`


1. [Deploy to Cloudflare](https://deploy.workers.cloudflare.com/?url=https://github.com/andesco/add-magnet-link) or use the [Cloudflare Dashboard](https://deploy.workers.cloudflare.com/?url=https://github.com/andesco/add-magnet-link).

2. Set these variables and secrets for your qBittorrent API:\
   Text: `API_BASE_URL`\
   Secret: `API_USERNAME`\
   Secret: `API_PASSWORD`

3. Visit the worker URL to authenticate: \
  `https://add.{user}.workers.dev`

## Add Magnet Link via URL

**full magnet link**:\
`https://add.{user}.workers.dev/magnet:?xt=urn:btih:{infohash}`
...
Required Variable | Description | Type
------------------|-------------|-----
`API_BASE_URL` | qBittorrent API URL | Text

Optional Variable | Description | Type
------------------|-------------|-----
`API_USERNAME` | qBittorrent API username (bypasses web login) | Secret
`API_PASSWORD` | qBittorrent API password (bypasses web login) | Secret

### Authentication

**Default (Web Login)**: Users authenticate via a web form using their qBittorrent credentials. Authentication is saved for 1 year via a secure cookie.

**Optional (Environment Variables)**: Set `API_USERNAME` and `API_PASSWORD` as secrets to bypass web login. This is useful when securing your worker with Cloudflare Access or for personal deployments.
